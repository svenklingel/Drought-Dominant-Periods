...
class PlotTimeAnalysisImpacts(TimeAnalysisImpacts):
    ...
    def read_median_total_counts(
        ...
        filenames = glob.glob(
            ...
            root_dir=os.path.join(OUTPUT_PATH + "event_counts" + self.impact_type),
        )
        ...
        for location, local_data in container.items():
            ...
            for event, specific_data in local_data.items():
                ...
                for t_0, data in specific_data.items():
                    ...
                    if (data["dominant_ret_per"] is not None or data['AR dominant period'] is not None) and USE_AR_MODEL:
                        plt.title(
                            f"{ISIMIP_IMPACT_NAME[self.impact_type]}\n "
                            f"dominant per. (AR)= {data['dominant_ret_per']}y ({data['AR dominant period']}y)",
                            fontsize=font,
                        )
    ...
    def create_global_plots(
        ...
        for event in extreme_ds_t0.data_vars.keys():
            ...
            world = gpd.read_file(os.path.join(os.getcwd(), "data", "ne_110m_admin_0_countries.zip"))
