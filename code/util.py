...
ISMIP_GCM_COLOR = {
    "gfdl-esm4": "tab:red",
    "ukesm1-0-ll": "tab:purple",
    "ipsl-cm6a-lr": "tab:blue",
...
WORLD_REGIONS = ['WLD', 'NAC', 'LCN', 'ECS', 'EAS', 'SAS', 'MEA', 'SSF']
FOURIER_MARKERSTYLE = {
    # "Fourier fit": "",  # solid
    4: "^",  # dotted
    3: "v",  # dashed
...
if os.path.exists(SURFACE_AREA_PATH):
    #surface_area = xr.load_dataarray(SURFACE_AREA_PATH)
...
def get_surface_area():
    print("get_surface_area function called")
    if os.path.exists(SURFACE_AREA_PATH):
        print("SURFACE AREA PATH EXISTS")
        return xr.load_dataarray(SURFACE_AREA_PATH)

    else:
        print("SURFACE AREA PATH DOES NOT EXIST")
        surface_area = np.array(
            [
                [
                    shape(
